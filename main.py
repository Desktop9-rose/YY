...
class MedicalApp(MDApp):
    def build(self):
        ...
        # Add screens manually to control ScreenManager logic if needed
        # But Builder already created structure, we just need to set it up
        # Since KV root is not ScreenManager, we need to restructure slightly
        # To make it simple, I will use the Builder to return ScreenManager
    ...
    def update_result_ui(self, data):
        ...
        sc.ids.res_title.text = data.get('title', '未命名')
        sc.ids.res_core.text = data.get('core_conclusion', '无')
        sc.ids.res_abnormal.text = str(data.get('abnormal_analysis', '无'))
        sc.ids.res_advice.text = str(data.get('life_advice', '无'))
